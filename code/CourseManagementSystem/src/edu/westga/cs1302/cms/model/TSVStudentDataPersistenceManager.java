...
/**
 * Supports saving and loading student data, with CSV (Comma Separated Values)
 * 
 * @author CS 1302
 * @version Fall 2024
 */
	...
	public void saveStudentData(Student[] students) throws IOException {
		...
		try (FileWriter writer = new FileWriter(DATA_FILE)) {
			for (Student currStudent : students) {
				if (currStudent != null) {
					writer.write(currStudent.getName() + REGEX + currStudent.getGrade() + System.lineSeparator());
				}
			}
		}
	...
	public Student[] loadStudentData() throws FileNotFoundException, IOException {
		...
		try (Scanner reader = new Scanner(inputFile)) {
			for (int lineNumber = 1; reader.hasNextLine(); lineNumber++) {
				...
				String[] parts = strippedLine.split(REGEX);
				try {
					String name = parts[0];
